  ...
  return (
    <div className="space-y-4">
      ...
      <div
        ...
        style={{
          border: `2px dashed ${file ? 'var(--charcoal)' : 'var(--grey)'}`,
          borderRadius: '0',
          padding: '2rem',
          background: file ? 'var(--grey-input)' : 'transparent'
        }}
      >
        <input
          ref={fileInputRef}
        ...
        {isExtracting ? (
          <div className="flex flex-col items-center gap-3">
            <svg className="animate-spin h-8 w-8" style={{ color: 'var(--charcoal)' }} viewBox="0 0 24 24">
              <circle
          ...
          <div className="flex flex-col items-center gap-2">
            <svg className="h-8 w-8" style={{ color: 'var(--green)' }} fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <p className="text-body font-medium">{file.name}</p>
            <p className="text-small">Click to upload a different file</p>
          </div>
        ) : (
          <div className="flex flex-col items-center gap-3">
            <svg className="h-10 w-10" style={{ color: 'var(--grey-dark)' }} fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {extractError && (
        <div className="alert alert-error">
          <p className="text-body">{extractError}</p>
        </div>
      )}
